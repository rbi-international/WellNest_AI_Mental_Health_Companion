...
from app.services.mood_analytics import MoodAnalyticsService, AnalyticsConfig
...
async def mood_analytics(
    ...
    config = AnalyticsConfig(window_days=days)

    service = MoodAnalyticsService(mood_repo=repo, cfg=config)

    return await service.compute(user_id=user_id)